...
const PLAYER_JWT_SECRET = process.env.PLAYER_JWT_SECRET || process.env.JWT_SECRET || 'sp_club_player_secret_key_2024';
...
const getApprovedPlayerByEmail = async (email) => Registration.findOne({
  ...
  try {
    ...
    if (player.playerForcePasswordReset) {
      return res.status(403).json({
        message: 'Account locked after multiple failed attempts. Please reset password using Email OTP.'
      });
    }

    const isValidPassword = await verifyCurrentPassword(player, password);
    if (!isValidPassword) {
      player.playerFailedLoginAttempts = Number(player.playerFailedLoginAttempts || 0) + 1;
      ...
      if (player.playerForcePasswordReset) {
        return res.status(403).json({
          message: 'Maximum wrong attempts reached. Password reset is now required via Email OTP.'
        });
      }
    ...
    if (player.playerFailedLoginAttempts || player.playerForcePasswordReset || player.playerLastFailedLoginAt) {
...
router.post('/password/forgot/request', async (req, res) => {
  try {
    ...
    if (!player) {
      return res.json({ message: 'If the account exists, OTP has been sent to registered email' });
    }
    ...
    player.playerPasswordResetOtpExpiresAt = new Date(Date.now() + 10 * 60 * 1000);
...
router.post('/password/forgot/reset', async (req, res) => {
  try {
    ...
    if (!player.playerPasswordResetOtpHash || !player.playerPasswordResetOtpExpiresAt) {
      return res.status(400).json({ message: 'OTP not requested or expired' });
    }

    if (new Date(player.playerPasswordResetOtpExpiresAt).getTime() < Date.now()) {
      return res.status(400).json({ message: 'OTP has expired. Request a new OTP.' });
    }

    const otpMatches = await bcrypt.compare(String(otp).trim(), player.playerPasswordResetOtpHash);
    if (!otpMatches) {
      return res.status(400).json({ message: 'Invalid OTP or email' });
    }
...
router.patch('/password/change', playerAuth, async (req, res) => {
  try {
    ...
    const player = await Registration.findById(req.playerId).select('_id phone playerPasswordHash');
    if (!player) {
...
router.get('/me', playerAuth, async (req, res) => {
  try {
    const player = await Registration.findById(req.playerId)
      .select('_id name email role idCardNumber phone parentsPhone aadharNumber dob bloodGroup gender address clubDetails photo certificates')
      .lean();
    ...
    return res.json({
      player: {
        ...
        photo: player.photo || '',
        certificates: Array.isArray(player.certificates) ? player.certificates : []
      }
    });
  } catch (error) {
...
router.post('/messages', playerAuth, async (req, res) => {
  try {
    ...
    const player = await Registration.findById(req.playerId)
      .select('_id name email phone idCardNumber');
...
router.put('/me/photo', playerAuth, upload.single('photo'), async (req, res) => {
  try {
    if (!req.file?.path) {
    ...
    if (oldPublicId && oldPublicId !== newPublicId) {
      try {
        ...
        console.error('Failed to delete old profile photo from Cloudinary:', destroyError);
      }
    }
...
router.post('/attendance/mark', playerAuth, async (req, res) => {
  try {
    ...
    const existingRecordIndex = player.attendance.findIndex((entry) => entry.date === today);
