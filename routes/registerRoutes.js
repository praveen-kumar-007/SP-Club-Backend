    ...
    try {
      ...
      if (req.body.kabaddiPositions) {
        try {
          ...
          return res
            .status(400)
            .json({ message: "Invalid kabaddiPositions format." });
        }
      }
      ...
      if (!req.files?.aadharFront?.length) {
        return res
          .status(400)
          .json({ message: "Aadhar front image is required." });
      }
      if (!req.files?.aadharBack?.length) {
        return res
          .status(400)
          .json({ message: "Aadhar back image is required." });
      }
